...
interface ConfigFile {
  schemaVersion: 1;
  golden_sources: GoldenSourceConfig[];
}

export class FsConfigStore implements IConfigStore {
  /** @param configPath absolute path of `golden-source-sync.config.json` (repo root, committed). */
  constructor(private readonly configPath: string) {}

  async loadAll(): Promise<GoldenSourceConfig[]> {
    return (await this.readFile()).golden_sources;
  }

  async upsert(config: GoldenSourceConfig): Promise<void> {
    const file = await this.readFile();
    const i = file.golden_sources.findIndex((c) => c.name === config.name);
    if (i >= 0) file.golden_sources[i] = config;
    else file.golden_sources.push(config);
    await this.writeFile(file);
  }

  async remove(name: string): Promise<void> {
    const file = await this.readFile();
    file.golden_sources = file.golden_sources.filter((c) => c.name !== name);
    await this.writeFile(file);
  }
  ...
  private async readFile(): Promise<ConfigFile> {
    try {
      ...
      if ((error as NodeJS.ErrnoException).code === 'ENOENT') {
        return { schemaVersion: 1, golden_sources: [] };
      }
      throw error;
    }
  }
